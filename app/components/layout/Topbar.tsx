...
export function Topbar() {
    ...
    // Basic title mapping logic assuming exact routes for now
    ...
    return (
        <header className="w-full flex items-center justify-between h-20 px-10 mb-6 mt-2">
            <h1 className="text-white text-2xl font-semibold">{title}</h1>

            <div className="flex items-center gap-4">
                {pathname.includes("/stable-swap") && (
                    <button className="flex items-center gap-2 bg-[#1a1a1a] bg-opacity-80 border border-[#333] hover:border-[#444] text-gray-300 px-4 py-2.5 rounded-xl text-sm font-medium transition-colors">
                        <OasisLogo />
                        Oasis Sapphire
                    </button>
                )}

                <button className="flex items-center gap-3 bg-[#1e1114] bg-opacity-80 border border-[#3a1d24] hover:border-[#522933] text-gray-300 px-4 py-2 rounded-xl transition-colors text-left">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-gray-400 mt-0.5 leading-tight">Create AI Wallet +</span>
                    </div>
                </button>
            </div>
        </header>
